...
function loadImage(url) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = 'anonymous'; // Это может помочь с CORS, но не всегда
...
function populateBlogs(items, id) {
  ...
  items.slice(0, count).forEach(async (item) => {
    ...
    try {
      const imgSrc = await loadImage(item.thumbnail);
...
populateProjects(embeddedProjects, "embedded-projects");
